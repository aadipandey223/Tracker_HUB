    ...
    <Card className="hover:shadow-xl transition-all duration-300 border-purple-200 dark:border-purple-900 bg-gradient-to-br from-purple-50 to-white dark:from-gray-800 dark:to-gray-900">
      <CardHeader>
        <CardTitle className="flex items-center justify-between">
          <span className="flex items-center gap-2">
            ...
            {t('dashboard.weeklyPlanner')}
          </span>
          <span className="text-sm font-normal text-gray-500 dark:text-gray-400">{t('dashboard.thisWeek')}</span>
        </CardTitle>
      </CardHeader>
      <CardContent className="space-y-6">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="p-4 rounded-lg bg-purple-50 dark:bg-purple-900/20 border border-purple-200 dark:border-purple-800">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-sm text-gray-600 dark:text-gray-400">{t('dashboard.tasks')}</span>
            </div>
            <p className="text-2xl font-bold text-purple-600">
              {completedTasks}/{weekTasks.length}
            </p>
            <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
              {weekProgress.toFixed(0)}% {t('dashboard.complete')}
            </p>
          </div>
          ...
          <div className="p-4 rounded-lg bg-indigo-50 dark:bg-indigo-900/20 border border-indigo-200 dark:border-indigo-800">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-sm text-gray-600 dark:text-gray-400">{t('dashboard.habits')}</span>
            </div>
            <p className="text-2xl font-bold text-indigo-600">
              {completedHabits}/{totalHabits}
            </p>
            <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
              {habitProgress.toFixed(0)}% {t('dashboard.complete')}
            </p>
          </div>
        </div>
        ...
        <div className="space-y-3">
          <h4 className="text-sm font-semibold text-gray-700 dark:text-gray-300">{t('dashboard.dailyActivity')}</h4>
        ...
        <div className="pt-4 border-t border-gray-200 dark:border-gray-700">
          <div className="grid grid-cols-3 gap-3">
            <div className="text-center">
              ...
              <p className="text-xs text-gray-500 dark:text-gray-400">{t('dashboard.completed')}</p>
            </div>
            <div className="text-center border-x border-gray-200 dark:border-gray-700">
              <p className="text-xl font-bold text-indigo-600">
                {Math.round((weekProgress + habitProgress) / 2)}%
              </p>
              <p className="text-xs text-gray-500 dark:text-gray-400">{t('dashboard.avgProgress')}</p>
            </div>
            <div className="text-center">
              <p className="text-xl font-bold text-blue-600">
                {dailyData.filter(d => d.total > 0).length}
              </p>
              <p className="text-xs text-gray-500 dark:text-gray-400">{t('dashboard.activeDays')}</p>
            </div>
          </div>
        </div>
      </CardContent>
